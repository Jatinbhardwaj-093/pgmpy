...
def run_mixture_benchmark(edge_prob=None, seed=42, l_penalties=None):
    ...
    print(f"m in {m_values} | n in {n_values} | Lambdas = {l_penalties}")
    print("=" * 120)
    ...
    for m in m_values:
        ...
        for n in n_values:
            ...
            df, true_dag = generate_synthetic_sem(n_nodes=m, edge_prob=prob, n_samples=n, seed=seed)
            print(f"Ground-Truth Edges count: {len(true_dag.edges())}")
            ...
            # Test L0 and L1 penalties for Complete vs Moral across each lambda
            test_cases = [
                ("Complete", "l0", None),
                ("Complete", "l1", None),
                ("Moral", "l0", ek_moral),
            ...
            for lp in l_penalties:
                for case_name, pen, ek in test_cases:
                    print(f"  Running {case_name} (penalty={pen}, lambda={lp})...")
    ...
    print("\n" + "=" * 120)
    print("8-GRAPH MIXTURE BENCHMARK SUMMARY TABLES (GROUPED BY SEARCH SPACE & PENALTY TYPE)")
    print("=" * 120)

    # Group by Configuration, Penalty, Lambda
    groups = results_df.groupby(["Configuration", "Penalty", "Lambda"], sort=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lambda-pen", type=float, nargs="+", default=[0.1, 1.0], help="Regularization penalty lambda(s)"
    )
